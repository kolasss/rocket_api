      ...
      module Supervisor
        class AssignCourier < ::Operations::V1::Base
          ...
          def call(id:, courier_id:)
            ...
            queue_auto_decline(courier.id, order.id)
            Success(order)
          end
          ...
          def validate_assignment(order, courier)
            ...
            elsif order.courier_assignments.where(courier_id: courier.id)
                       .exists?
          ...
          AUTO_DECLINE_TIMEOUT = 5.minutes

          def queue_auto_decline(courier_id, order_id)
            CourierAssignmentAutodeclineWorker.perform_in(
